...
public final class main extends JavaPlugin implements Listener
{
    ...
    private void chunkCleansingCheck(Chunk chunk, int x, int z, int y)
    {
        ...
        if (totality == 0)
        {
            if (length > Global.configMinEntityWarning)
            {
                if (Global.configToggleAlertChunkWarning)
                {
                    Component message = Component.text("§c■ " + "§cCHUNK WARNING§7: §eFound §ax" + length +" §centities near§7: §a[" + x + ", " + y + ", " + z + "§a]" + " §c■")
                            .clickEvent(ClickEvent.copyToClipboard(x + " " + y + " " + z))
                            .hoverEvent(HoverEvent.showText(Component.text("§aClick to copy coordinates.")));
